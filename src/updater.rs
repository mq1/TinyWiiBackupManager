...
pub fn check_for_updates() -> Result<()> {
    ...
    if current_version != latest_release {
        let result = MessageDialog::new()
            ...
            .set_description(format!("A new version of TinyWiiBackupManager is available: {}.\nDo you want to download it?", latest_release))
            .set_buttons(rfd::MessageButtons::YesNo)
            .show();

        if result == MessageDialogResult::Yes {
            open::that(RELEASES_BASE_URL.to_owned() + &latest_release)?;
        }
    }
