...
use crate::{
    ...
    ui::{UiAction, games_grid, games_list},
};
...
pub fn update(ctx: &egui::Context, app_state: &AppState, ui_buffers: &mut UiBuffers) {
    egui::CentralPanel::default().show(ctx, |ui| {
        ...
        match app_state.config.contents.view_as {
            ViewAs::Grid => games_grid::update(ui, app_state, ui_buffers),
            ViewAs::List => games_list::update(ui, app_state, ui_buffers),
        }
    });
}
...
fn update_top_bar(ui: &mut egui::Ui, app_state: &AppState, ui_buffers: &mut UiBuffers) {
    ui.horizontal(move |ui| {
        ...
        group.show(ui, |ui| {
            ...
            if ui
                .add(
                    egui::TextEdit::singleline(&mut ui_buffers.games_filter)
                        .desired_width(200.)
                        .hint_text("Search by Title/ID"),
                )
                .changed()
            {
                ui_buffers.action = Some(UiAction::ApplyFilterGames);
            }
        });
        ...
        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
            ...
            if ui
                ...
                .clicked()
            {
                ui_buffers.action = Some(UiAction::TriggerRefreshGames);
            }
            ...
            let group = egui::Frame::group(ui.style()).fill(ui.style().visuals.extreme_bg_color);
            group.show(ui, |ui| {
                ui.checkbox(&mut ui_buffers.show_gc, "🎲")
                    .on_hover_text("Show GameCube");

                ui.separator();

                ui.checkbox(&mut ui_buffers.show_wii, "🎾")
