            ...
            move |action, args: &str, action_queue: &mut Vec<(Action, SharedString)>| {
                ...
                match action {
                        ...
                        if let Some(in_path) = dialogs::pick_wiiload(&window_handle) {
                            ...
                            std::thread::spawn(move || {
                                ...
                                let _ = weak.upgrade_in_event_loop(move |logic| match res {
                                    Ok(msg) => logic.invoke_dispatch(
                                        Action::NotifyInfo,
                                        msg.to_shared_string(),
                                    ),
                                    Err(e) => {
                    ...
                    Action::OscContentsCached => {
                        ...
                        let displayed_apps =
                            new.iter().map(DisplayedOscApp::from).collect::<Vec<_>>();
                    ...
                    Action::InstallOscApp => {
                        ...
                        std::thread::spawn(move || {
                            ...
                            let _ = weak.upgrade_in_event_loop(move |logic| {
                                if let Err(e) = res {
                                    logic
                                        .invoke_dispatch(Action::NotifyError, e.to_shared_string());
                                } else {
                                    let msg =
                                        slint::format!("{} installed successfully", &app.name);
                    ...
                    Action::NormalizeDirLayout => {
                        match normalize_dir_layout::perform(&config.contents.mount_point) {
                            ...
                            Err(e) => {
                                let msg =
                                    slint::format!("Failed to normalize directory layout: {e}");
                                notifications.push(Notification::error(msg));
                            }
                        }
                    }
                    ...
                    Action::DownloadAllCovers => {
                        ...
                        let _ = std::thread::spawn(move || {
                            let res = if for_wiiflow {
                                ...
                                twbm_core::covers::download_all_covers_for_usbloadergx(
                                    &ids, &config,
                                )
                            };
