...
pub fn read_disc_header<R: Read>(reader: &mut R) -> Option<(Format, String, String)> {
...
pub fn guess_format<R: Read>(reader: &mut R) -> Result<(Format, Vec<u8>)> {
    let mut id_buf = vec![0u8; 6];
...
pub fn read_disc_header_from_game_dir(path: &Path) -> Option<(Format, String, String)> {
